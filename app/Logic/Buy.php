...
class Buy
{
    ...
    private function _createOrderStep1()
    {
        ...
        foreach ($this->_order_data['checkedGoodsList'] as $k => $va) {
            if ($va['number']) {
                if (empty($va['goods_specifition_name_value'])) {
                    $goods_specifition_name_value = $va->checked_products->goods_spec_item_names ?? '';
                } else {
    ...
    public function makePaySn($member_id)
    {
        ...
        return mt_rand(10, 99) . sprintf('%010d', time() - 946656000) . sprintf('%03d',
                (float)microtime() * 1000) . sprintf('%03d', (int)$member_id % 1000);
    }
